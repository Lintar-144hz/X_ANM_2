...
import { SiteSettings, Student, OrganizationMember, ScheduleItem, ContentItem, MediaFile } from '@shared/types';
import { Sparkles, ArrowRight, Users, Award, Calendar, Newspaper, Clapperboard, Layers, ChevronRight, CheckCircle2, Zap, Crown, Image as ImageIcon, Maximize2 } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen bg-slate-100 text-slate-950 font-sans selection:bg-amber-400 selection:text-slate-950">
      ...
      <section className="py-12 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto pb-24">
        <div className="bg-white border-3 border-slate-950 rounded-3xl p-6 sm:p-10 shadow-[6px_6px_0px_0px_rgba(15,23,42,1)]">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-8 gap-4">
            <div>
              <span className="bg-amber-300 text-slate-950 font-black text-[10px] uppercase tracking-widest px-3 py-1 rounded-md border-2 border-slate-950 inline-block mb-2 font-mono shadow-[2px_2px_0px_0px_rgba(15,23,42,1)]">
                DOKUMENTASI & KARYA TERBARU
              </span>
              <h2 className="text-2xl sm:text-4xl font-black uppercase text-slate-950 tracking-tight">
                Media
              </h2>
            ...
            <button
              ...
              className="text-xs font-black uppercase tracking-wider text-slate-950 bg-amber-400 hover:bg-amber-300 flex items-center gap-1 group px-5 py-2.5 rounded-xl border-3 border-slate-950 shadow-[3px_3px_0px_0px_rgba(15,23,42,1)] transition-all cursor-pointer self-start sm:self-auto"
            >
          ...
          {mediaList.length === 0 ? (
            <div className="text-center py-16 bg-slate-50 rounded-2xl border-2 border-slate-950 text-slate-600 text-xs font-mono font-bold">
              Belum ada foto media dalam galeri.
            </div>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
              {mediaList.slice(0, 4).map((item) => (
                <div
                  key={item.id}
                  onClick={() => onNavigate('/galeri')}
                  className="bg-white border-3 border-slate-950 rounded-2xl overflow-hidden group shadow-[4px_4px_0px_0px_rgba(15,23,42,1)] hover:translate-x-[-2px] hover:translate-y-[-2px] hover:shadow-[6px_6px_0px_0px_rgba(15,23,42,1)] transition-all cursor-pointer flex flex-col justify-between"
                >
                  <div className="aspect-[16/10] bg-slate-100 overflow-hidden relative border-b-3 border-slate-950">
                    <img
                      src={item.url}
                      alt={item.name}
                    ...
                    <div className="absolute top-2 left-2">
                      <span className="px-2 py-0.5 rounded bg-white text-slate-950 border-2 border-slate-950 text-[9px] font-mono font-black uppercase shadow-[2px_2px_0px_0px_rgba(15,23,42,1)]">
                        {item.category || 'Karya'}
                      </span>
                    </div>
                  </div>

                  <div className="p-4 space-y-2.5 flex-1 flex flex-col justify-between bg-amber-50/40">
                    <div>
                      <h4 className="font-black text-sm text-slate-950 group-hover:text-amber-600 transition-colors line-clamp-1">
                        {item.name}
                      </h4>
                      {item.description ? (
                    ...
                    <div className="pt-2.5 border-t-2 border-slate-950 flex items-center justify-between text-[11px] text-slate-700 font-mono font-bold">
                      ...
                      <span className="text-slate-950 font-black group-hover:underline">Lihat &rarr;</span>
                    </div>
                  </div>
                </div>
              ))}
